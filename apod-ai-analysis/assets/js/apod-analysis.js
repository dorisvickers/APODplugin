...
jQuery(document).ready(function($) {
    ...
    function renderAIAnalysis(containerId, analysis) {
        const container = document.getElementById(containerId);
        if (container) {
            const root = createRoot(container);
            root.render(<VertexAnalysis analysis={analysis} />);
        }
    }
    ...
    function updateResultsGrid(data) {
                    ...
                    ${sortedResults.map(result => `
                        <div class="apod-result-card">
                            ...
                            <div class="apod-content-wrapper">
                                ...
                                <div id="ai-analysis-${result.date.replace(/-/g, '')}" class="ai-analysis"></div>
                            </div>
                        </div>
                    `).join('')}
                </div>
        ...
        // Initialize AI analysis for each result
        sortedResults.forEach(result => {
            renderAIAnalysis(
                `ai-analysis-${result.date.replace(/-/g, '')}`,
                result.aiAnalysis
            );
        });
