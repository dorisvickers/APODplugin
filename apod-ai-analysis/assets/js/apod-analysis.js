jQuery(document).ready(function($) {
    ...
    function findSimilarImages(target, allImages, count = 3) {
        return allImages
            ...
            .map(img => {
                const targetCat = determineCategory(target.title, target.explanation);
                const imgCat = determineCategory(img.title, img.explanation);
    ...
    function updateStatistics(data) {
        ...
        data.results.forEach(result => {
            const category = determineCategory(result.title, result.explanation);
    ...
    function updateResultsGrid(data) {
                    ...
                    ${sortedResults.map(result => `
                        <div class="apod-result-card">
                            ...
                            <div class="apod-content-wrapper">
                                ...
                                <div class="apod-category">${categoryLabels[determineCategory(result.title, result.explanation)]}</div>
        ...
        // Add event handlers for show more buttons
        ...
        // Add event handlers for AI explanation buttons
    ...
    function filterResults(searchTerm, category) {
        ...
        if (category && category !== 'all') {
            results = results.filter(result => 
                determineCategory(result.title, result.explanation) === category
            );
        }
